...
function test_cipher_cbc {
	...
	while read i
	do
 	    $BASE/sym_crypt -i $TEST/ref/$i  -o $TEST/crypted/cbc_${k}_${i} -k $k -m cbc-crypt -v azertyuiopqsdfgh&> /dev/null
...
function test_decipher_cbc {
	...
	do
	    while read i
	    do
 		$BASE/sym_crypt -o $TEST/decrypted/cbc_${k}_$i  -i $TEST/crypted/cbc_${k}_${i} -k $k -m cbc-uncrypt -v azertyuiopqsdfgh&> /dev/null
